...
double NetworkNode::GetActiveTime() {
	return m_activeTime;
}
...
#endif
	/*
	 if (m_nodeType == NetworkNode::TYPE_HOME_BASE_STATION && p->GetNPackets()) {
	 std::cout << "Send " << p->GetNPackets() << " packets by femto @ "
	 << Simulator::Init()->Now() << "s" << ", State is " << m_state
	 << ", size is ";
	...
	for (it = packets.begin(); it != packets.end(); it++) {
		...
		if (packet->GetDestinationMAC() == GetIDNetworkNode()) {
			if (packet->GetRLCHeader() == NULL) {
				...
				int rlcEntityIndex =
						packet->GetRLCHeader()->GetRlcEntityIndex();
				RadioBearerInstance* bearer =
						GetProtocolStack()->GetRrcEntity()->GetRadioBearer(
								rlcEntityIndex);
