...
/* My method to pause the simulation */
void
Simulator::RunAndStop (int time)
{
	  m_stop = false;
	std::cout << "RunAndStop" << std::endl;
	/* At each loop, the time will decrease by one, 
	 * when it reaches to zero, the loop break
	 */
	while (!m_calendar->IsEmpty () && !m_stop && time-- != 0)
	{
		ProcessOneEvent ();
...
//  cout << "unscheduledEvents: " << --m_unscheduledEvents << endl;
//	cout << "Time Stemp: " << m_currentTs << endl;
//	cout << "Uid: " << m_currentUid << endl;
