...
static void SingleCellWithFemto(double radius, int nbBuildings,
	...
	double duration = stop_time; //used to be 100
...
//  }
	...
	MyTraffic MyTrafficApplication[nbMy * totalNbUE];
	...
	for (iter3 = userEquipmentContainer->begin();
		...
		for (int j = 0; j < nbMy; j++) {
			MyTrafficApplication[myApplication].SetSource(gw);
			MyTrafficApplication[myApplication].SetDestination(ue);
			MyTrafficApplication[myApplication].SetApplicationID(applicationID);
			MyTrafficApplication[myApplication].SetStartTime(start_time);
			MyTrafficApplication[myApplication].SetStopTime(duration_time);
			MyTrafficApplication[myApplication].SetInterval(time_miu);
			MyTrafficApplication[myApplication].SetSize(file_size_miu);

			//MyTrafficApplication[myApplication].SetSBitRate(bitrate);
			...
			MyTrafficApplication[myApplication].SetClassifierParameters(cp);

			QoSParameters *qos = new QoSParameters();
			MyTrafficApplication[myApplication].SetQoSParameters(qos);
...
//	Simulator::Init()->Schedule(10, &NetworkNode::MakeActive, oldTarget);
	...
	simulator->Run();
	std::cout << "number of transmission is "
			<< MyTrafficApplication->on_period_num << std::endl;
	std::cout << "off time is " << Simulator::off_time << std::endl;
	std::cout << "in time is " << Simulator::in_time << std::endl;
