...
#include "../flows/application/RandomBurst.h"

static void RandomBurstTest(double radius, int nbBuildings,
		int buildingType, double activityRatio,
		 int nbUE, int nbFemtoUE,
		int nbVoIP, int nbVideo, int nbBE,int nbCBR, int nbRB, int sched_type,
		int frame_struct, int speed, int accessPolicy, double maxDelay,
		int videoBitRate, int seed, int filesize, int nb_transmission,
		double my_miu, double idle_time, double sleep_time) {
        double stop_time = nb_transmission * my_miu;
	int nbCell = 1;
	// define simulation times
	double duration = stop_time;
	double flow_duration = duration;
	int cluster = 3;
	double bandwidth = 5;

	int numberoffemtocell = 0;

	// CREATE COMPONENT MANAGER
	Simulator *simulator = Simulator::Init();
	FrameManager *frameManager = FrameManager::Init();
	NetworkManager* nm = NetworkManager::Init();
	Simulator::idle_time = idle_time;
	Simulator::sleep_time = sleep_time;
	...
	for (int idBuilding = 0; idBuilding < nbBuildings; idBuilding++) {
		...
		nm->CreateBuildingForFemtocells(idBuilding, buildingType, apartmentSide,
				nbFloors, buildingCenter_X, buildingCenter_Y,
	...
	RandomBurst RBApplication[nbRB * totalNbUE];
	...
	//For each cell, add nbUE number of UE
	for (int j = 0; j < nbCell; j++) {

		//users are distributed uniformly into a cell
		vector<CartesianCoordinates*> *positions = GetUniformUsersDistribution(
				j, nbUE); //position is calculated
		...
		for (int i = 0; i < nbUE; i++) {
			...
			double speedDirection = (double) (rand() % 360)
					* ((2 * 3.14) / 360);
			;

			// cell == eNBs
			UserEquipment* ue = new UserEquipment(idUE, posX, posY, speed,
					speedDirection, cells->at(j), eNBs->at(j), 1, //HO activated!
			...
			// Both user in marco and femto cell are added
			...
			eNBs->at(j)->GetPhy()->GetDlChannel()->GetPropagationLossModel()->AddChannelRealization(
					c);
	...
	for (int j = 0; j < nbFemtoCells; j++) {
		...
		for (int i = 0; i < nbFemtoUE; i++) {
			...
			UserEquipment* ue = new UserEquipment(idUE, posX, posY, speed,
					speedDirection, femtocells->at(j), HeNBs->at(j), 1, //HO activated!
					Mobility::RANDOM_DIRECTION);
			...
			c->SetChannelType(ChannelRealization::CHANNEL_TYPE_PED_A);
			HeNBs->at(j)->GetPhy()->GetDlChannel()->GetPropagationLossModel()->AddChannelRealization(
					c);
	...
	for (iter3 = userEquipmentContainer->begin();
		...
		double start_time = 0.01; // + GetRandomVariable (5.);
		...
		int file_size_miu = filesize;
		double time_miu = my_miu;


		for (int j = 0; j < nbRB; j++) {
			RBApplication[rbApplication].SetSource(gw);
			RBApplication[rbApplication].SetDestination(ue);
			RBApplication[rbApplication].SetApplicationID(applicationID);
			RBApplication[rbApplication].SetStartTime(start_time);
			RBApplication[rbApplication].SetStopTime(duration_time);
			RBApplication[rbApplication].SetInterval(time_miu);
			RBApplication[rbApplication].SetSize(file_size_miu);

			//RBApplication[rbApplication].SetSBitRate(bitrate);
			...
			RBApplication[rbApplication].SetClassifierParameters(cp);

			QoSParameters *qos = new QoSParameters();
			RBApplication[rbApplication].SetQoSParameters(qos);
	...
	simulator->Run();
	std::cout <<  Simulator::off_time * sleep_time / stop_time/numberoffemtocell << "\n";
}
